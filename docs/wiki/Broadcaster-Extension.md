...
- **Stage via CI on `main`.** When `backend/broadcaster-extension/` changes, [`.github/workflows/stage-chrome-extension.yml`](https://github.com/AlecMcCutcheon/collabfm-radio/blob/main/.github/workflows/stage-chrome-extension.yml) uploads a ZIP to the Developer Dashboard automatically (**upload only** — not submitted for review). Skips if a version is already **PENDING_REVIEW**. **Submit for review manually** in the Developer Dashboard when you are finished iterating — intentional, so you choose when Chrome review starts.
- **Review delay.** After you submit for review, expect **Chrome Web Store review** before the public listing updates (the Go live modal may show store version lag until then).

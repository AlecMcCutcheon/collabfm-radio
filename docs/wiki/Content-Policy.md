...
**The policy engine is a filtering tool, not a copyright detector.** It **implements best-effort filtering based on metadata** using your configured allowlists and fallbacks on reported source, track, and license metadata. It does not analyze audio, fingerprint tracks, verify licenses, or determine whether content is legally cleared to stream.

...
When track and license metadata are available, listeners see source and license links under now-playing on the web UI and in the live session log. CollabFM does not host or provide audio—it relays what broadcasters supply. FMA and Jamendo are default allowed sources because the extension can read machine-readable license metadata there—not because those catalogs are guaranteed safe to broadcast. Default allowed licenses cover standard Creative Commons terms (CC BY, CC BY-SA, CC BY-NC, CC BY-NC-SA, CC BY-ND, CC BY-NC-ND, CC0); admins must still verify attribution, share-alike, and other obligations for their use case.
